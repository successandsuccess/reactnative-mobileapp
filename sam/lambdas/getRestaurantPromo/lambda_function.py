...
def lambda_handler(event, context):
    ...
    if (customerId):
        ...
        if (len(promoStruct['id']) > 0):
            found = getPromo(promoStruct)
        # elif (testing):
        #     found = createPromoInJustDB(promoStruct['restaurant_id'], promoStruct)
        else:
            found = createPromo(promoStruct['restaurant_id'], promoStruct, customerId)
...
def getPromo(promoDetails, dynamodb=None):
    if not dynamodb:
        dynamodb = boto3.resource('dynamodb')

    table = dynamodb.Table(PROMOS_DATA_TABLE)
...
def createPromo(restaurantId, promoDetails, stripeCustomerId, dynamodb=None):
...
def createPromoInJustDB(restaurantId, promoDetails, dynamodb=None):
    if not dynamodb:
        dynamodb = boto3.resource('dynamodb')
    table = dynamodb.Table('Promotions')
